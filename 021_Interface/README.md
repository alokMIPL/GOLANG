...
| Concept | Explanation |
|---|---|
| **Interface** | A set of method signatures; any type implementing them satisfies the interface automatically. |
| **Implicit satisfaction** | No `implements` keyword needed — Go checks structurally at compile time. |
| **Dependency injection** | `payment` receives its gateway from the outside (via the `gateway` field) rather than creating it internally. |
| **Value receivers** | All `pay` methods use value receivers (`(r razorpay)`, not `(r *razorpay)`), so struct values (not pointers) satisfy the interface. |
| **Decoupling** | `payment` never references `razorpay`, `stripe`, or `paypal` by name in its logic (once the bug above is fixed) — it only knows about the `paymenter` interface. |

---

## Possible Extensions

If you want to build on this example further:

1. **Return errors from `pay`**: Real payment gateways can fail (network errors, invalid amounts, etc.). Consider changing the signature to `pay(amount float32) error`.
2. **Add a `Refund` method**: Extend the interface (or create a second interface) to support refunds, demonstrating interface composition.
3. **Use interface composition**: e.g., combine `Payer` and `Refunder` into a `PaymentProcessor` interface.
4. **Add unit tests**: Use `fakepayment` as a mock gateway to test `payment.makePayment` without hitting real gateway logic — this is exactly why the fake gateway type exists.
5. **Export types**: If this were to become a reusable package, rename to `Paymenter`, `Payment`, `Pay`, etc. (capitalized) so they're accessible outside the package.
6. **Naming convention**: Consider renaming `paymenter` to something like `Gateway` or `PaymentGateway` — `paymenter` is a slightly awkward name; interfaces are conventionally named as either agent nouns ("-er" suffix on the action, like `Payer`) or descriptive nouns.
